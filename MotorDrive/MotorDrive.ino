...
void LadderUp(char* command, byte comLen)
{
	...
	MotorUL.set(leftSpeed);
	MotorUR.set(rightSpeed);

        MotorDL.set(rightSpeed);
	MotorDR.set(leftSpeed);
}
...
void Ride(char* command, byte comLen)
{
	...
	MotorWL.set(leftSpeed);
	MotorWR.set(rightSpeed);
}
...
void setup()
{
	...
	parser.AddCommand("ride", 8, Ride);
        parser.AddCommand("ladder", 8, LadderUp);
}
